...
\documentclass[11pt,                        % Schriftgröße
               ...
               BCOR0.0cm,                   % Rand zum Binden
               openbib,						% Erzeugt Absätze im Literaturverzeichnis
               ngerman,
               ]{scrreprt}
%-----------------------------------------------------------------
% Einbinden verschiedener nützlicher Pakete
%---------------------------------
\usepackage[ansinew]{inputenc}  % Ermöglicht die Verwendung von Umlaute
                                % und Sonderzeichen: 'ansinew'  für Windows
                                % 'latin1' für Linux
...
\usepackage{varioref}
\usepackage{xcolor}             % Farbige Darstellung von Wörtern
\usepackage{framed}
\usepackage{graphicx}           % ermöglicht das Einbinden von Grafikdateien
...
\usepackage{array}              % Für komplexe Tabellen
...
\usepackage{multirow}           % Für die Tabellen um mehrere Spalten zusammenzufassen
\usepackage{eurosym}            % Eurosymbol
\usepackage{url}                % Verwenden von URLs
\urlstyle{tt}
\usepackage[geometry]{ifsym}    % Für Symbole
\usepackage[intlimits,sumlimits]{amsmath} % erweiterte Formeln, Optionen
                                % ermöglichen die Grenzen oberhalb und
                                % unterhalb zu setzen
\usepackage{amssymb}            % blackboard Buchstaben (z.B. Symbol für komplexe Zahlen IC)
\usepackage{theorem}            % Emöglicht die Definition von eigenen Umgebungen
...
%---------------------------------
\input{Einstellungen.tex}               % Sammlung von Abkürzungen für Mathematiksymbolen
                                % (benötigt die Pakete amssymb und theorem)
...
%\include{Titel_Master}
%\include{Titel_Bachelor}
\include{Titel_Projekt}
...
%    \include{Kapitel/Kurzfassung2}		
    \include{Kapitel/Personen}
...
%    \include{Kapitel/Stand_der_Technik_Systemkonzept}
    \include{Kapitel/Hauptteil}
%    \include{Kapitel/Zusammenfassung}
    \include{Kapitel/Zusammenfassung2}

    %#################################################################
    % Anhang
    % ----------------------------------------------------------------
 %   \appendix
   \include{Kapitel/Anhang/Quellenverzeichnis}
 %   \include{Kapitel/Anhang/Erklaerung_DA}

 %   \chapter{Nomenklatur}
 %   \section{Abkürzungen}
 %       \input{Kapitel/Anhang/Abkuerzungen.tex}
 %   \section{Symbole und Formelzeichen}
 %       \input{Kapitel/Anhang/Formelzeichen.tex}

    \bibliographystyle{Alphadin}    % Alphabetisch sortiertes Literaturverzeichnis mit
                                    % Einordnungsmarken aus Verfasser und
                                    % Erscheinungsjahrkürzeln
